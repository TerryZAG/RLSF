...
import numpy as np
from torch.utils.tensorboard import SummaryWriter  # add by zt
class BaseTrainer:
    """
    ...
    def train(self):
        ...
        all_trgs = []
        # add by zt
        summary_writer = SummaryWriter(log_dir=(self.checkpoint_dir), comment='_one_hour_sleepedf')
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # add by zt
            # log['epoch'],['loss'],['accuracy'],['val_loss'],['val_accuracy'],
            log_loss = log['loss']
            log_accuracy = log['accuracy']
            log_val_loss = log['val_loss']
            log_val_accuracy = log['val_accuracy']
            summary_writer.add_scalar('loss', log_loss, epoch)
            summary_writer.add_scalar('accuracy', log_accuracy, epoch)
            summary_writer.add_scalar('val_loss', log_val_loss, epoch)
            summary_writer.add_scalar('val_accuracy', log_val_accuracy, epoch)
        ...
        summary_writer.close()  # add by zt
